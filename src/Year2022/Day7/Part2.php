...
class Part2
{
    ...
    private int $smallest = PHP_INT_MAX;
    ...
    private const FS_SIZE_TOTAL = 70000000;
    private const FS_SIZE_REQUIRED = 30000000;
    ...
    public function solve(string $puzzle): int
    {
        ...
        $currentFreeSpace = self::FS_SIZE_TOTAL - $this->fileSystem[self::FS_SIZE];
        $missingSpace = self::FS_SIZE_REQUIRED - $currentFreeSpace;

        $this->readDirectoryRecursive($this->fileSystem, $missingSpace);

        return $this->smallest;
    }

    private function readDirectoryRecursive(array $directory, int $missingSpace): void
    {
        if ($directory[self::FS_SIZE] >= $missingSpace &&
            $directory[self::FS_SIZE] < $this->smallest)
        {
            $this->smallest = $directory[self::FS_SIZE];
        }

        foreach ($directory[self::FS_SUB_DIRECTORIES] as $subDirectory) {
            $this->readDirectoryRecursive($subDirectory, $missingSpace);
        }
    }
